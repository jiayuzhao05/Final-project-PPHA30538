...
我会依次介绍探索的问题 用的哪些数据 如何建模 以及目前初步结果和简单的streamlit demo

三个研究问题：
> 一，新闻情绪能不能预测州级银行基本面的恶化？ 这里的基本面主要用 FDIC 的 ROA 等盈利指标来度量。
> 二，相比传统的 FDIC 指标，比如历史 ROA、NIM、资产和存款增速，新闻情绪是不是更早、更强地反映未来的压力？
> 三，这种压力是不是有明显的地理集聚——是不是总是集中在某些州

方法上 借鉴CTR/CVR 框架：
- 在广告里，CTR 是‘会不会点’，CVR 是‘点了之后会不会转化’，两者相乘是 eCPM。
- 在这个项目里，把 bad_year 看成 CTR：某个州在某一年是不是进入‘坏年份’；
- 把 severity 看成 CVR：一旦是坏年份，ROA 会恶化到什么程度；
- 两者相乘就得到州级的 StressScore

Data
A:新闻情绪数据
- 来源是一个金融新闻/分析师标题的数据集 analyst_ratings_processed.csv，里面有标题、时间戳和对应的股票。
- 我先用 all-data.csv 里带标签的文本，训练了一个非常轻量的情绪分类器：TF‑IDF 特征加 Logistic 回归，大概区分正面、负面、中性。
...
> - 在这个基础上我构造了 ROA、ΔROA 等一系列监管视角下的基本面指标。
> \n所有清洗之后的结果会合并到一个 state_year_panel 里，后面的静态图和 Streamlit 都是基于这个面板做的。”
...
> 用每个州自己的历史分布来定义‘坏年份’：
> - 如果某一年的ΔROA 掉到这个州历史的 20% 分位数以下，就记作 bad_year = 1，否则是 0；
...
> - 纵轴是下一期的ΔROA。
> \n可以看到，点云整体是有一点向右下方延伸的趋势：
> - 在负面情绪比较高的区域，ΔROA 更有可能变成负数，也就是下一年 ROA 出现明显恶化。
> 这张图还不是严格的因果推断，但它给了一个比较直观的信号：
...
> - 在预测 severity 的 R² 上，目前整体还比较弱，属于非常早期的 baseline。

> 第一，情绪确实包含一些信号，但用这么粗的‘按年聚合’和简单情绪模型，信息还是被稀释了；
> 第二，模型本身还比较原始，我现在用的是线性模型，只是把 AuroraBid 的 CTR/CVR 框架迁移过来，并没有真正实现 Wide&Deep 或 PPO/Bandit。

这次 presentation 只是‘work in progress’的 checkpoint，final 报告里会尝试两件事情：
> - 把情绪做得更细一点，例如按季度甚至更短窗口聚合；
> - 换成Wide&Deep 结构，看看非线性模型能不能把这些弱信号放大出来。”
...
> - 下面还有一个表格，列出当前年份各州的主要指标，方便按 StressScore 排序、对比。

> 后续如果时间允许，会在 app 里加入：
> - 不同监测策略的比较，比如按历史 ROA 排 vs 按 StressScore 排，
> - 以及一个简化的 Bandit 策略，模拟监管在有限注意力下怎样挑选最值得盯的州。