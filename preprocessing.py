...
def build_news_sentiment_yearly(
    ...
    *,
    ...
    for chunk in pd.read_csv(news_path, usecols=usecols, chunksize=chunksize):
        ...
        chunk["YEAR"] = dt.dt.year.astype("Int64")
...
def make_static_plots(panel: pd.DataFrame) -> None:
    ...
    try:
        ...
        if not year_candidates.empty:
            ...
            if not map_df.empty:
                gdf = gdf_base.merge(
                    map_df[["STNAME", "StressScore"]], left_on="NAME", right_on="STNAME", how="left"
                )
                ...
                ax.set_axis_off()
                ax.set_title(f"Geographic stress map — StressScore {map_year}")
                fig.tight_layout()
        ...
        if not hist.empty:
            bad_share = (
                hist.groupby("STNAME", as_index=False)["bad_year"]
                .mean()
            ...
            fig, ax = plt.subplots(figsize=(12, 7))
            gdf2.plot(
                column="bad_year_share",
                ax=ax,
                legend=True,
                cmap="OrRd",
                vmin=0.0,
                vmax=gdf2["bad_year_share"].max(),
                missing_kwds={"color": "lightgrey", "label": "Missing"},
            )
            ax.set_axis_off()
            ...
            fig.tight_layout()
            fig.savefig(OUTPUTS_DIR / "static_map_badyear_share_labeled.png", dpi=200)
            plt.close(fig)
    except Exception:
